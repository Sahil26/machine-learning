...
a2=[ones(size(a2),1), a2]; % size 5000* 26
z3=a2*Theta2';
a3=sigmoid(z3); %size 5000* 10
b=y_matrix.*log(a3)+(1-y_matrix).*log(1-a3);
l=sum(b);
J=(-1/m)*(sum(l')); %unregularized cost
temp1=Theta1.*Theta1;
t1=sum(temp1);
temp2=Theta2.*Theta2;
t2=sum(temp2);
J= J + (lambda/(2*m))*(sum(t1')+sum(t2') -t1(1,1) - t2(1,1));

gre=sigmoidGradient(z2);
delta3=a3-y_matrix;
delta2=(delta3*Theta2(:, 2:end)).*gre; % 5000*25
